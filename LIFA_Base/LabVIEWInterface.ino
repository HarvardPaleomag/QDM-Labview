  ...
  if(command[0] == 0xFF && checksum_Test(command) == 0)
  {
    ...
    *********************************************************************************/
    case 0x34:  // IR Transmit  
      ...
      break;
    case 0xFE:  // SRF04 and SRF05 (Mode 1)
      digitalWrite(command[2], LOW);  // Set the trigger pin to low for 2uS
      delayMicroseconds(2);
      digitalWrite(command[2], HIGH);  // Send a 10uS high to trigger ranging
      delayMicroseconds(10);
      digitalWrite(command[2], LOW);  // Send pin low again
      retVal = pulseIn(command[3], HIGH, 35000);  // Read in times pulse
